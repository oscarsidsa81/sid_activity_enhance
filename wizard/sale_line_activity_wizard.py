from odoo import api, fields, models, _
from odoo.exceptions import UserError


class SaleLineActivityWizard(models.TransientModel):
    _name = 'sale.line.activity.wizard'
    _description = 'Batch assign activities to sale order lines'

    operation = fields.Selection([
        ('add', 'Add selected activities'),
        ('remove', 'Remove selected activities'),
    ], required=True, default='add')
    user_id = fields.Many2one('res.users', string='Responsible')
    date = fields.Date(string='Activity Date', default=fields.Date.context_today)
    description = fields.Char(string='Description', default='Revisar instrucciones en campo Comments de la linea de venta')
    activity_type_ids = fields.Many2many('sid.activity.tag', string='Activity Types')
    line_ids = fields.Many2many('sale.order.line', string='Sale Lines', readonly=True)
    preview_activity_ids = fields.Many2many('sale.activity', string='Activities in selected lines', compute='_compute_existing_activities', readonly=True)

    @api.depends('line_ids')
    @api.depends_context('active_ids', 'active_model')
    def _compute_existing_activities(self):
        for wiz in self:
            lines = wiz.line_ids
            if not lines and wiz.env.context.get('active_model') == 'sale.order.line':
                lines = wiz.env['sale.order.line'].browse(wiz.env.context.get('active_ids', [])).exists()
            acts = wiz.env['sale.activity'].sudo().search([('sale_line_id', 'in', lines.ids)])
            wiz.preview_activity_ids = [(6, 0, acts.ids)]

    @api.model
    def default_get(self, fields_list):
        vals = super().default_get(fields_list)
        if self.env.context.get('active_model') == 'sale.order.line':
            active_ids = self.env.context.get('active_ids', [])
            vals['line_ids'] = [(6, 0, active_ids)]
        return vals

    def action_apply(self):
        self.ensure_one()
        sale_lines = self.line_ids.exists() or self.env['sale.order.line'].browse(self.env.context.get('active_ids', [])).exists()
        if not sale_lines:
            raise UserError(_('No sale order lines selected.'))
        selected_types = self.activity_type_ids.mapped('code')
        if not selected_types:
            raise UserError(_('Select at least one activity type.'))
        Activity = self.env['sale.activity'].sudo()
        if self.operation == 'add':
            for line in sale_lines:
                for act_type in selected_types:
                    if Activity.search_count([('sale_line_id', '=', line.id), ('type', '=', act_type)]):
                        continue
                    Activity.create({
                        'sale_line_id': line.id,
                        'type': act_type,
                        'user_id': self.user_id.id or False,
                        'date': self.date,
                        'description': self.description or 'Revisar instrucciones en campo Comments de la linea de venta',
                    })
        else:
            acts = Activity.search([('sale_line_id', 'in', sale_lines.ids), ('type', 'in', selected_types)])
            acts.unlink()
        return {'type': 'ir.actions.act_window_close'}
